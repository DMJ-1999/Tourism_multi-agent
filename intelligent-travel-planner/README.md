...
基于LangChain多智能体架构的智能旅行规划系统，使用**高德地图API**获取真实景点/酒店数据、**美团开放平台API**获取餐饮数据，通过多Agent协作实现个性化的旅行规划服务。
...
- **多智能体协作**: 4个专业Agent协同工作
  ...
  - 🍜 餐饮推荐 — 美团开放平台API搜索餐厅，推荐当地美食
  - 💰 预算审计员 — 计算总费用，智能优化预算分配

- **真实数据驱动**: 通过高德地图Web服务API获取实时POI数据
  - 景点搜索：高德地图POI搜索（风景名胜、博物馆、古迹等）
  - 酒店搜索：高德地图POI搜索（酒店、宾馆、民宿等）
  - 餐厅搜索：美团开放平台商户搜索（中餐厅、火锅、小吃等）
...
| 餐饮信息 | **美团开放平台商户搜索** | 实时搜索餐厅，含菜系、评分、人均、招牌菜 |
...
- **高德地图API**: 实时POI数据（景点/酒店搜索）
- **美团开放平台API**: 真实餐饮数据（菜系、评分、人均消费、招牌菜）
- **Gradio**: Web交互界面
- **Pydantic**: 数据验证和模型定义
- **pytest**: 测试框架
